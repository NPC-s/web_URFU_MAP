...
from points.models import ClassRoom, MapImages
...
def rename_page(request : HttpRequest):
    if not request.user.is_superuser: # type: ignore
        return HttpResponseRedirect("/")
    if request.method == "POST":
        ...
        class_room.save()
    return render(request, "./Admin/RenamePoints.html", {"points" : get_all_classrooms("RI", 2)})
